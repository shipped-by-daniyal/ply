...
Figma (variables + sticker sheet + Token Usage table)
  → /ply-export-tokens → DTCG JSON (with $description + usage $extensions)
...
| `.claude/` | Claude Code commands (`/ply-*`) and subagents (design-advisor, docs-writer) |
...
- Every non-trivial choice gets an ADR (`/ply-adr`) **before** implementation.
- Every phase of work ends with `/ply-update-docs` + a commit.
- Token work starts with `/ply-sync-tokens` to catch Figma↔repo drift.
